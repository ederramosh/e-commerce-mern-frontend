  ...
  return (
    <>
      {getItems.map((item, key) => {
        return (
          <div className="card col-4 px-3 mx-3 my-3 shadow p-3 mb-5 bg-body rounded" style={{ width: "20rem" }} key={key}>
            <img src={item.itemImage} className="card-img-top mt-3" alt="..." />
            <div className="card-body">
              <h5 className="card-title">{item.name}</h5>
              <p className="card-text">{item.features}</p>
              <p className="card-text fs-5">Precio: ${item.price.toFixed(2)}</p>
